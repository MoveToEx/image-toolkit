...
function ResolutionAlignmentOptions({ onConfirm, onCancel, context }: ResolutionAlignmentOptionsProps) {
  ...
  const [boxTag, setBoxTag] = useState(false);
  ...
  return (
    <div className="flex flex-col gap-4 py-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            <div className='flex items-center gap-3'>
              <Checkbox id='box-tag' checked={boxTag} onCheckedChange={val => setBoxTag(val === 'indeterminate' ? false : val)} />
              <Label htmlFor='box-tag'>Add box tag</Label>
            </div>
          </div>
      ...
      <div className="flex justify-end gap-2 mt-4">
        ...
        <Button onClick={() => onConfirm({ width, height, position, color, boxTag })}>Execute</Button>
      </div>
    </div>
  );
}
