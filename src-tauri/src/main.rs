...
use image_toolkit_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m image_toolkit`,
    // i.e, run the `src-tauri/python/image_toolkit/__main__.py`
    let py_script = PythonScript::Module("image_toolkit".into());
